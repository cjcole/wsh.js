...
${opts.join("\n")}`;
    ...
    _load() {
        ...
        this.setCommandHandler("help", {
            desc: "Print help",
            exec: async (/* cmd, opts, args */) => {
                return templates.help({ commands: this._commands() });
            }
        });
        ...
        this.setCommandHandler("history", {
            ...
            exec: async (cmd, opts/* , args */) => {
                ...
                return templates.history({ items: this.history.items() });
            }
        });

        this.setCommandHandler("_default", {
            exec: async (cmd/* , opts, args */) => {
                return templates.badCommand({ cmd: cmd });
            },
            completion: async (cmd, argName, arg/* , line */) => {
                ...
                return util.bestMatch(arg, this._commands());
            }
        });
        ...
        this.readline.on("completion", (line, callback) => {
            ...
            handler.completion(cmd, argName, arg, line)
                .then((match) => {
